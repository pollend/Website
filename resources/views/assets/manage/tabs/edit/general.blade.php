			...
			<div class="row">
				<div class="col-sm-12">
					<div class="form-group">
                        ...
                        <textarea name="description" placeholder="Description" id="description" class="form-control markdown"
                                  rows="15">{{ Request::old('description', $asset->description) }}</textarea>
			...
			@foreach($secondaryTags as $index => $tag)
				<div class="checkbox">
					<input id="tag-{{ $tag->slug }}" name="tags[{{ $tag->slug }}]" type="checkbox"
					       @if(Request::old('tags.'.$tag->slug, $asset->tags->where('id', $tag->id)->count()>0) != false) checked @endif>

...
namespace PN\Forum\Http\Controllers\API;
...
class ApiPostController extends ApiBaseController
{
    ...
    {
        $this->validate($request, ['thread_id' => ['required'], 'user_id' => ['required']]);

        $thread = Thread::find($request->input('thread_id'));
        $this->authorize('reply', $thread);

        $post = $this->model()->create($request->only(['thread_id', 'post_id', 'user_id', 'title', 'content']));
        $post->load('thread');
